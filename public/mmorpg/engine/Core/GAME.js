...
export class Game_start {
    ...
    update() {
        if (
            ...
            !Game.scene.all_scenes[Game.scene.current].ready
        ) {
            Game.scene.all_scenes[Game.scene.current].start();
        ...
        if (
            ...
            window.getComputedStyle(UI.Inputs.sendMessage).display != 'none'
        ) {
            UI.Inputs.sendMessage.style.display = 'none';
            UI.Inputs.sendMessageInput.blur();
        } else if (
            Game.frameInput == 'external' &&
            window.getComputedStyle(UI.Inputs.sendMessage).display != 'block'
        ) {
